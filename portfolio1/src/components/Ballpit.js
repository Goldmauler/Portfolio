...
class W {
  ...
  #R() {
    ...
    for (let i = 1; i < e.count; i++) {
      const s = 3 * i;
      // Random positions - start them higher up
      t[s] = E(2 * e.maxX);
      t[s + 1] = k(-e.maxY * 0.5, e.maxY); // Start in upper half
      t[s + 2] = E(2 * e.maxZ);
      
      // Give strong initial upward velocities
      v[s] = E(0.2);
      v[s + 1] = k(0.05, 0.25); // Strong upward push
      v[s + 2] = E(0.1);
    }
  }
  setSizes() {
    ...
    for (let idx = r; idx < t.count; idx++) {
      ...
      for (let jdx = idx + 1; jdx < t.count; jdx++) {
        ...
        if (dist < sumRadius) {
          ...
          // Reduced collision response - balls push away more gently
          H.copy(j).multiplyScalar(Math.max(B.length(), 0.5) * 0.3);
          T.copy(j).multiplyScalar(Math.max(N.length(), 0.5) * 0.3);
      ...
      if (t.controlSphere0) {
        ...
        if (dist < sumRadius0) {
          ...
          // Much stronger push away from cursor - this makes balls float up
          H.copy(j).multiplyScalar(Math.max(B.length(), 4) * 2);
...
const X = {
  ...
  followCursor: true  // Changed to true so cursor pushes balls
};
...
class Z extends d {
  constructor(e, t = {}) {
    ...
    const materialConfig = {
      envMap: n,
      metalness: i.materialParams?.metalness ?? 0.5,
      roughness: i.materialParams?.roughness ?? 0.5,
      clearcoat: i.materialParams?.clearcoat ?? 1,
      clearcoatRoughness: i.materialParams?.clearcoatRoughness ?? 0.15
    };
    const r = new Y(materialConfig);
    if (r.envMapRotation) {
